...
async function getStepDefinitionPaths(stepDefinitionPatterns) {
    const files = (await Promise.all(stepDefinitionPatterns.map((pattern) => util_1.default.promisify(glob_1.default)(pattern, { nodir: true })))).reduce((acum, el) => acum.concat(el), []);
    if (files.length === 0) {
...
function getStepDefinitionPatternsPost10(configuration, filepath) {
    ...
    const filepathReplacement = trimFeatureExtension(path_1.default.relative(configuration.preprocessor.implicitIntegrationFolder, filepath));
    ...
    (0, debug_1.default)(`looking for step definitions using ${util_1.default.inspect(stepDefinitions)}`);
    return stepDefinitions
        ...
        .map((pattern) => path_1.default.join(projectRoot, pattern));
}
...
function getStepDefinitionPatternsPre10(configuration, filepath) {
    const fullIntegrationFolder = (0, helpers_1.ensureIsAbsolute)(configuration.cypress.projectRoot, configuration.cypress.integrationFolder);
