...
async function compile(configuration, data, uri = this.resourcePath) {
    ...
    const ensureRelativeToProjectRoot = (path) => (0, helpers_1.ensureIsRelative)(configuration.projectRoot, path);
    return `
    ...
    const registry = withRegistry(() => {
      ${stepDefinitionPaths
        .map((stepDefinitionPath) => `require(${stringify(stepDefinitionPath)});`)
        .join("\n    ")}
    });
